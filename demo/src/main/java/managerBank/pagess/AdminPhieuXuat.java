...
public class AdminPhieuXuat extends  JFrame{
    JButton dash;
    JButton nhapKho;
    JButton xuatKho;
    JButton PhieuNhap;
...
for (int i = 0; i < tongPhieuNhap.getColumnCount(); i++) {
    tongPhieuNhap.getColumnModel().getColumn(i).setCellRenderer(centerRenderer);
        ...
        for (PhieuXuatDto phieu : listPhieuInfor){
            model.addRow(new Object[]{phieu.getMa_phieu_xuat(), phieu.getNgay_xuat(), phieu.getNguoi_xuat(), phieu.getTong_hoa_don()});
